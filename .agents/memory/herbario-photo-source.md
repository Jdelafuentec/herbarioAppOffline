...
Species photos are referenced as remote `{ uri }` URLs (no bundling). Trade-off: needs network; fine for now, revisit if offline field use is requested. Exception: the three paisaje-type images (bofedal/vega/pajonal) are user-provided aerial photos bundled locally under the app's assets (`ImageSourcePropType` via `require`), replacing the old remote OASIS sets — so `Paisaje.images` are NOT strings.
